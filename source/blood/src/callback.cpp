...
void fxBloodBits(int nSprite) // 14
{
    ...
    if (pSprite->ang == 1024)
    {
        const char bDudeStillScreaming = !spriRangeIsFine(pSprite->owner) || (sprite[pSprite->owner].statnum != kStatThing) || !(xvel[pSprite->owner]>>12 || yvel[pSprite->owner]>>12);
        int nChannel = 28+(pSprite->index&2);
        dassert(nChannel < 32);
        if (VanillaMode() || bDudeStillScreaming) // only trigger splat sfx if enemy is alive, or dead and stopped moving (prevents enemy sfx cutoff)
