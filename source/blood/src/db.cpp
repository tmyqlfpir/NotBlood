...
int dbRandomizerRNG(const int range)
{
    ...
    if (gGameOptions.nRandomizerMode >= 2) // if pickups or enemies+pickups mode, randomize pickup
    {
        ...
        case kThingObjectGib: // generic breakable objects
        {
            ...
            switch (pSprite->picnum)
            {
                ...
                {
                    if (dbRandomizerRNG(2)) // lower chance of replacing object
                        break;
                ...
                {
                    if (dbRandomizerRNG(5)) // lower chance of replacing object
                        break;
    ...
    *pSector = mapHeader.ate;
    ...
    for (int i = 0; i < mapHeader.at23; i++)
    {
        ...
        if (sprite[i].extra > 0)
        {
            ...
            if (gGameOptions.nRandomizerMode && !VanillaMode() && !DemoRecordStatus() && (gGameOptions.nRandomizerCheat != -1) && !pXSprite->scale && !dbRandomizerRNG(3)) // if not in demo/vanilla mode, and randomizer is on and random seed cheat is being used
            {
                switch (pSprite->type) // make enemies randomly huge
                {
                    case kDudeRat:
                        pXSprite->scale = clamp(dbRandomizerRNG(2048-128)+128, 128, 2048);
                        break;
                    case kDudeHand:
                        pXSprite->scale = clamp(dbRandomizerRNG(1024-128)+128, 128, 1024);
                        break;
                    case kDudeHellHound:
                        pXSprite->scale = clamp(dbRandomizerRNG(512-128)+128, 128, 512);
                        break;
                    ...
                    case kDudeGargoyleStone:
                        pXSprite->scale = clamp(dbRandomizerRNG(512-32)+32, 32, 512);
                        break;
                    case kDudeGargoyleFlesh:
                        pXSprite->scale = clamp(dbRandomizerRNG(512-32)+32, 32, 512);
                        break;
                    ...
                    case kDudeCultistTommyProne:
                    case kDudeCultistShotgunProne:
                        pXSprite->scale = clamp(dbRandomizerRNG(384-128)+128, 128, 384);
                        break;
                    case kDudeCultistTNT:
                        pXSprite->scale = clamp(dbRandomizerRNG(384-96)+96, 96, 256);
                        break;
                    case kDudeZombieButcher:
                    case kDudeGillBeast:
                        pXSprite->scale = clamp(dbRandomizerRNG(384-64)+64, 64, 384);
                        break;
                    default:
                        break;
                }
