...
static void ircd_parse(ircd_t *cl, char *buf, int len)
{
    ...
    else
    {
        ...
        else if (Bsscanf(buf, ":%[^!]!%s %s %s :%[^\r^\n]", from, mask, cmd, dest, text) >= 3)
        {
            ...
            {
                if (!bSelf) // someone new joined, inform them that we have a game available
                {
                ...
                else // we finally joined the channel
                {
...
int netIRCIinitialize(void)
{
    ...
    Bsprintf(m_ircd.nick, gNetMode == NETWORK_SERVER ? "BlSrv%ld%X" : "BlCli%ld%X", (unsigned int)(time(NULL)/60), (unsigned char)(time(NULL)&0xF));
