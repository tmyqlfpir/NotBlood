...
void viewBurnTime(int gScale)
{
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        int xoffset = scale(burnTable[i].nX-(320>>1), 320>>1, 266>>1); // scale flame position
...
RORHACK:
        ...
        if (gViewPos == 0)
        {
            ...
            if (!gWeaponInterpolate) // if position interpolate weapon is off, crunchify the weapon positions
            {
                v4c >>= 8;
                v4c <<= 8;
                v48 >>= 8;
