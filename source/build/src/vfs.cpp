...
static int32_t kopen4loadinsensitive(const char *filename, char searchfirst)
{
#ifdef _WIN32
    ...
    if (h == buildvfs_kfd_invalid) // if file failed to open, reattempt using case sensitive filename for case sensitive file systems
    {
        int32_t len = -1;
        for (int32_t i = 0; filename[i] != '\0'; i++) // find last slash in filename
        {
            if (filename[i] == '/' || filename[i] == '\\') // update bookmark
                len = i;
        }

        BDIR *dir = NULL;
        struct Bdirent *dirent = NULL;
        char *filepath = NULL;
        if (len >= 0) // filename has directory slashes
        {
            ...
            if (filename[0] == '.') // filepath already has dot and slash, use as is
            {
                for (int32_t i = 0; i < len; i++) // copy string
                    filepath[i] = filename[i];
            }
            else if (filename[0] == '/' || filename[0] == '\\') // filepath starts with slash, insert a dot at the beginning
            {
                filepath[0] = '.';
                for (int32_t i = 0; i < len; i++) // copy string with offset for dot
                    filepath[i+1] = filename[i];
            }
            else // filepath is likely checking current directory, add a dot and slash
            {
                filepath[0] = '.';
                filepath[1] = '/';
                for (int32_t i = 0; i < len; i++) // copy string with offset for dot and slash
                    filepath[i+2] = filename[i];
            }
        }
        else if (filename[0] != '\0') // filename is likely within current directory
        {
            filepath = (char *)Balloca(3);
            filepath[0] = '.';
            filepath[1] = '/';
            filepath[2] = '\0';
            len = 0;
        ...
        dir = Bopendir(filepath);
        if (!dir) // directory doesn't exist, abort
            return h;

        char bFound = 0;
        while ((dirent = Breaddir(dir)) != NULL)
        {
            for (int32_t i = 0;; i++)
            {
                char const bDiff = Btolower((unsigned char)dirent->name[i]) - Btolower((unsigned char)filename[len+i]);
                if ((bDiff != 0) || (dirent->name[i] == '\0'))
                {
                    bFound = bDiff == 0;
                    break;
                }
        ...
        int32_t pathlen = Bstrlen(filepath), filelen = Bstrlen(dirent->name);
        char *filepathcase = (char *)Balloca(pathlen+filelen); // allocate new case sensitive filepath
        filepathcase[pathlen] = filepathcase[pathlen+filelen] = filepathcase[0] = '\0';
        Bstrncat(filepathcase, filepath, pathlen);
        Bstrncat(filepathcase, dirent->name, filelen);
        Bclosedir(dir);

        lastpfn = NULL;
        h = kopen_internal(filepathcase, &lastpfn, searchfirst, 1, 1, newhandle, filegrp, filehan, filepos);
        Xfree(lastpfn);
    }
...
buildvfs_kfd kopen4loadfrommodinsensitive(const char *fileName, char searchfirst)
{
    ...
    if (g_modDir[0] != '/' || g_modDir[1] != 0)
    {
        static char staticFileName[BMAX_PATH];
        Bsnprintf(staticFileName, sizeof(staticFileName), "%s/%s", g_modDir, fileName);
        kFile = kopen4loadinsensitive(staticFileName, searchfirst);
    }

    return (kFile == buildvfs_kfd_invalid) ? kopen4loadinsensitive(fileName, searchfirst) : kFile;
}
